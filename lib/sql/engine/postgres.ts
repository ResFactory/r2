...
import * as pgQuery from "https://deno.land/x/postgres@v0.16.1/query/query.ts";
...
export function pgDatabaseConnConfig(
  ...
  const dbConnAxioms = ax.serDeAxioms({
    ...
    database: envBuilder.text("DATABASE", "PGDATABASE"),
    hostname: envBuilder.text("HOST", "PGHOST", "PGHOSTADDR"),
    port: envBuilder.integer("PORT", "PGPORT"),
    user: envBuilder.text("USER", "PGUSER"),
    password: envBuilder.text("PASSWORD", "PGPASSWORD"),
    dbConnPoolCount: envBuilder.integer("POOL_COUNT", "PGCONNPOOL_COUNT"),
  });
