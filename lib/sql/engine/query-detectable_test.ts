...
Deno.test("detect engine instance from query", async (tc) => {
  const ctx = SQLa.typicalSqlEmitContext();
  type Context = typeof ctx;
  const engine = mod.queryBasedEngine<"osquery", Context>((detected) => {
  ...
  await tc.step("bad spec test, misspelled USE DATABASE", () => {
    ...
    const result = engine.instance(ctx, badSpecTest);
  ...
  await tc.step("unknown engine instance ID, use default", () => {
    ...
    const result = engine.instance(ctx, badInstanceTest);
    ta.assert(result);
